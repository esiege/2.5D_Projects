...
function update(rootScope) {
	...
	requestUpdateFrame(function () {
		...
		for (var j = 0; j < updateFunctions.length; j++) {
			var func = updateFunctions[j];;
			
			
			if (!updateFunctions[j].params.persist) {
				updateFunctions.splice(j, 1);
				j--;
			}
			
			if (func.execute) {
				if (func.timeout) {
					if (!func.timeoutCounter) {
						func.timeoutCounter = func.timeout;
						func.execute(func.params);
					}
					else
						func.timeoutCounter--;
				}
