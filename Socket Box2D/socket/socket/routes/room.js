...
module.exports = function (socket) {
	...
	//socket.on('grid:sendUserToServer', function (data) {

	//	var room = rooms['room' + data.data.sid];
	//	if (room) {
	//		var user = room.users[data.data.localPos];

	//		user.position = data.data.user.position;

	//		socket.in('room' + data.data.sid).broadcast.emit('grid:updateUserFromServer',user);
	//	}
	//	else {
	//		log('room ' + data.data.sid + ' not found, possible rooms:', rooms);
	//	}
	...
	//socket.on('action:begin', function (data) {
	//	log('var', data.data.variableDetails);
	//	socket.in('room' + data.data.sid).emit('action:begin', { sid: data.data.sid, slot: 0, actionName: data.data.actionName, variableDetails: data.data.variableDetails, currentStep: data.data.currentStep });
	//	socket.in('room' + data.data.sid).broadcast.emit('action:begin', { sid: data.data.sid, slot: 1, actionName: data.data.actionName, variableDetails: data.data.variableDetails, currentStep: data.data.currentStep });
	//});
