...
executeQuery('DELETE from activeUsers');
...
module.exports = function ( socket )
{
    ...
    //check login
    onSocketCall_Select('login:getUserFromActive', 'activeUsers', 'userName', 'data');
    //add to logged in
    onSocketCall_Insert('login:addUserToActive', 'activeUsers');
    //delete active user
    onSocketCall_Delete('login:removeUserFromActive', 'activeUsers', '', 'data');
    //update active user
    onSocketCall_Update('login:updateActivePlayer1', 'activeUsers');


    //check other active accounts for matching
    //onSocketCall_Select( 'account:getMatchingUsers', 'activeUsers', 'userName', 'data' );

    //get session status
    onSocketCall_Select('login:getSessionStatus', 'activeUsers', '*', 'data');
    ...
    onSocketCall_Function( 'login:getSessionId', function ( data )
    {
        ...
        //--------------------------------------------------
        var sql = "SELECT * FROM " + prefix + "activeUsers WHERE userName != '" + data.data.userName + "' and status = 'Matching' and session_id is not NULL";
        executeAndCallbackSQLResults( sql, data, function ( data, results )
        {
            ...
            function addNewSession(callback)
            {
                ...
                var sql = "UPDATE " + prefix + "activeUsers SET session_id = " + sessionIdCounter + " WHERE userName = '" + data.data.userName + "'";
                executeAndCallbackSQLResults( sql, data, function ( data, results )
                {
            ...
            {
                var sql = "UPDATE " + prefix + "activeUsers SET session_id = " + player.session_id + " WHERE userName = '" + data.data.userName + "'";
                executeAndCallbackSQLResults( sql, data, function ( data, results )
                {
                    //session id copied from other player

                    var sql = "UPDATE " + prefix + "activeUsers SET status = 'Session Found' WHERE session_id = '" + player.session_id + "'";
                    executeAndCallbackSQLResults( sql, data, function ( data, results )
                    {
